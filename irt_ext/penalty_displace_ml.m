  function displace = penalty_displace_ml(offsets, sizes)
%|function displace = penalty_displace_ml(offsets, sizes)
%|
...
if nargin == 1 && streq(offsets, 'test'), penalty_displace_ml_test, return, end
...
end % penalty_displace_ml


% penalty_displace_ml_test()
function penalty_displace_ml_test

jf_equal(penalty_displace_ml([], [1]), nan(0,1))
jf_equal(penalty_displace_ml([], [1 1]), nan(0,2))

if 1 % check tiny
	% when nx=ny=2 then [1 nx nx+1 nx-1] = [1 2 3 1] so it is ambiguous!
	nx = 2; ny = 2;
	offsets = [1 nx nx+1 nx-1];
	dd = penalty_displace_ml(offsets, [nx ny]);
...
dd = penalty_displace_ml(offsets, [nx ny nz]);
jf_equal(dd, [ix(:) iy(:) iz(:)])

dim = [8 9]; % for won
offsets2 = penalty_offsets('2d:hvd', dim);
dd2 = penalty_displace_ml(offsets2, dim);

for nz=1:3
	dim3 = [dim nz];
	dd3 = penalty_displace_ml(offsets2, dim3);
...
if 1
	...
	[[1:13]', penalty_displace_ml(offsets, dim3)]
end

end % penalty_displace_ml_test()
