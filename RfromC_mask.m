  function R = RfromC_mask(varargin)
%|function R = RfromC_mask(varargin)
%|
...
if nargin == 1 && streq(varargin{1}, 'test'), RfromC_mask_test, return, end
...
meth = {
	'cgrad', @RfromC_mask_cgrad, '(R,x)';
	'penal', @RfromC_mask_penal, '(R,x)';
	'denom', @RfromC_mask_denom, '(R,x)';
	'C', @RfromC_mask_C, '()';
};
...
% RfromC_mask_penal()
function penal = RfromC_mask_penal(dummy, arg, x)
tmp = arg.C1 * x;
...
% RfromC_mask_C()
function C = RfromC_mask_C(dummy, R)
D = Gdiag(sqrt(arg.wt), 'mask', true(size(arg.wt)));
C = D * arg.C1;


function RfromC_mask_test
...
wt = 3;
R = RfromC_mask('pot', pot, 'C1', C1, 'wt', 3);
