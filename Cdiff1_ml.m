 function ob = Cdiff1_ml(isize, varargin)
%function C1 = Cdiff1_ml(isize, [options])
%|
%| Construct Cdiff1_ml object that can compute C1 * x and the adjoint C1' * d
...
%| However, it may be cpu dependent so use "Cdiff1_ml_tune" to compare.
%|
...
caller = ['Cdiff1_ml:' arg.type_diff];
Cdiff1_ml_Fatrix = @(arg, forw, back) ...
	Fatrix(arg.dim, arg, 'caller', caller, ...
		'forw', forw, 'back', back, ...
		'power', @Cdiff1_ml_power, 'abs', @Cdiff1_ml_abs);
Cdiff1_ml_fatrix2 = @(arg, does_many, forw, back) ...
...
case 'circshift'
	arg = Cdiff1_ml_coef_setup(arg);
	switch arg.class
	case 'fatrix2'
		ob = Cdiff1_ml_fatrix2(arg, true, ...
			@Cdiff1_ml_cs_forw, @Cdiff1_ml_cs_back);
	case 'Fatrix'
		ob = Cdiff1_ml_Fatrix(arg, ...
			@Cdiff1_ml_cs_forw_Fatrix, @Cdiff1_ml_cs_back_Fatrix);
	otherwise, fail bug
	end

case {'convn', 'imfilter'}
	arg = Cdiff1_ml_coef_setup(arg);
	arg = Cdiff1_ml_filt_setup(arg);
	switch arg.class
	case 'fatrix2'
...
case 'for1' % trick: just using "ind" for back because for1 is slow anyway
	switch arg.class
	case 'fatrix2'
		forw = @(arg, x) Cdiff1_ml_for1_forw(arg, x);
		back = @(arg, y) Cdiff1_ml_ind_back(arg, y); % !
...
case 'ind'
	switch arg.class
	case 'fatrix2'
		forw = @(arg, x) Cdiff1_ml_ind_forw(arg, x);
		back = @(arg, y) Cdiff1_ml_ind_back(arg, y);
		ob = Cdiff1_ml_fatrix2(arg, false, forw, back);
	case 'Fatrix'
		ob = Cdiff1_ml_Fatrix(arg, ...
...
case 'mex'
	...
	switch arg.class
	case 'fatrix2'
		forw = @(arg, x) Cdiff1_ml_mex_forw(arg, x);
		back = @(arg, y) Cdiff1_ml_mex_back(arg, y);
		ob = Cdiff1_ml_fatrix2(arg, true, forw, back);
	case 'Fatrix'
		ob = Cdiff1_ml_Fatrix(arg, ...
...
% Cdiff1_ml_abs()
% for abs(C)
function ob = Cdiff1_ml_abs(ob)
ob.arg.is_abs = true;


% Cdiff1_ml_power()
% for C.^2
function ob = Cdiff1_ml_power(ob, p)
...
% y = I * x
%
function y = Cdiff1_ml_ident_dup(arg, x)
y = x;


% Cdiff1_ml_coef_powabs()
function coef = Cdiff1_ml_coef_powabs(coef, arg)
...
function ii = Cdiff1_ml_i1(offset, nn)
ii = (1 + max(offset,0)) : (nn + min(offset,0));


% Cdiff1_ml_i2()
% indices for 2nd-order differences
function ii = Cdiff1_ml_i2(offset, nn)
offset = abs(offset);
...
%
function y = Cdiff1_ml_mex_forw(arg, x)
...
% Cdiff1_ml_mex_forw_Fatrix()
% y = C * x
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_mex_forw_Fatrix(arg, x)

[x ei] = embed_in(x, true([arg.isize 1]), arg.nn); % [(N) *L]
y = Cdiff1_ml_mex_forw(arg, x);
...
% both [(N) *L] (handles multi)
%
function x = Cdiff1_ml_mex_back(arg, y)
...
% Cdiff1_ml_mex_back_Fatrix()
% x = C' * y
% both [*N (L)] or [(N) (L)]
%
function x = Cdiff1_ml_mex_back_Fatrix(arg, y)

[y eo] = embed_out(y, arg.isize); % [(N) *L]
x = Cdiff1_ml_mex_back(arg, y);
...
% both [(N) *L] (does_many)
%
function y = Cdiff1_ml_cs_forw(arg, x)

displace = arg.displace;

switch arg.order
case 1
	coef = Cdiff1_ml_coef_powabs(-1, arg);
	y = x + coef * circshift(x, displace);

case 2
	coef = Cdiff1_ml_coef_powabs([2 -1], arg);
	y = coef(1) * x ...
...
% Cdiff1_ml_cs_forw_Fatrix()
% y = C * x
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_cs_forw_Fatrix(arg, x)

[x ei] = embed_in(x, true([arg.isize 1]), arg.nn); % [(N) *L]
y = Cdiff1_ml_cs_forw(arg, x);
y = ei.shape(y); % [*N (L)] or [(N) (L)]
...
% both [(N) *L]
%
function x = Cdiff1_ml_cs_back(arg, y)
arg.displace = -arg.displace; % trick
x = Cdiff1_ml_cs_forw(arg, y);


% Cdiff1_ml_cs_back_Fatrix()
% x = C' * y
% both [*N (L)] or [(N) (L)]
%
function x = Cdiff1_ml_cs_back_Fatrix(arg, y)
arg.displace = -arg.displace; % trick
x = Cdiff1_ml_cs_forw_Fatrix(arg, y);
...
% both [(N) *L] (does_many)
%
function y = Cdiff1_ml_diff_forw(arg, x)
...
% both [(N) *L]
%
function x = Cdiff1_ml_diff_back(arg, y)
if arg.diff_dim ~= 1
...
% Cdiff1_ml_for1_loop1()
% a small function for matlab to compile "just in time"
% both [(N)]
function out = Cdiff1_ml_for1_loop1(in, i1, i2, off, coef)
...
% Cdiff1_ml_for1_loop2()
% a small function for matlab to compile "just in time"
% both [(N)]
function out = Cdiff1_ml_for1_loop2(in, i1, i2, off, coef)
out = zeros(size(in), class(in));
switch class(in)
case 'single'
	coef = single(coef);
...
% both [(N)]
%
function y = Cdiff1_ml_for1_forw(arg, x)
...
case 1
	...
	if 1 % faster!
		y = Cdiff1_ml_for1_loop1(x, ...
			1+max(off,0), arg.nn+min(off,0), off, coef);
	else % slower!
		y = zeros(size(x), class(x));
		for ii = Cdiff1_ml_i1(off, arg.nn)
...
case 2
	coef = Cdiff1_ml_coef_powabs([2 -1], arg);
	if 1 % faster!
		y = Cdiff1_ml_for1_loop2(x, ...
			1+abs(off), arg.nn-abs(off), off, coef);
	else % slower!
		y = zeros(size(x), class(x));
		for ii = Cdiff1_ml_i2(off, arg.nn)
...
% Cdiff1_ml_for1_forw_Fatrix()
% y = C * x
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_for1_forw_Fatrix(arg, x)
...
y = zeros(size(x), class(x));
for ll = 1:ncol(x)
	y(:,ll) = Cdiff1_ml_for1_forw(arg, x(:,ll));
end
...
% both [(N)]
%
function y = Cdiff1_ml_ind_forw(arg, x)
...
case 1
	coef = Cdiff1_ml_coef_powabs(-1, arg);
	ii = Cdiff1_ml_i1(arg.offset, arg.nn);
...
case 2
	coef = Cdiff1_ml_coef_powabs([2 -1], arg);
	ii = Cdiff1_ml_i2(arg.offset, arg.nn);
	y(ii) = coef(1) * x(ii) + coef(2) * (x(ii-off) + x(ii+off));
...
% Cdiff1_ml_ind_forw_Fatrix()
% y = C * x
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_ind_forw_Fatrix(arg, x)
...
y = zeros(size(x), class(x));
for ll=1:ncol(x)
	y(:,ll) = Cdiff1_ml_ind_forw(arg, x(:,ll));
end
...
% both [(N)]
%
function x = Cdiff1_ml_ind_back(arg, y)
...
% Cdiff1_ml_ind_back_Fatrix()
% x = C' * y
% both [*N (L)] or [(N) (L)]
%
function x = Cdiff1_ml_ind_back_Fatrix(arg, y)
...
x = zeros(size(y), class(y));
for ll=1:ncol(y)
	x(:,ll) = Cdiff1_ml_ind_back(arg, y(:,ll));
end
...
% Cdiff1_ml_filt_forw_Fatrix()
% y = C * x
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_filt_forw_Fatrix(arg, x)

nd = length(arg.isize);

[x ei] = embed_in(x, true([arg.isize 1]), arg.nn); % [(N) *L]

coef = Cdiff1_ml_coef_powabs(arg.coef, arg);
...
% Cdiff1_ml_filt_back_Fatrix()
% x = C' * y
% both [*N (L)] or [(N) (L)]
%
function x = Cdiff1_ml_filt_back_Fatrix(arg, y)
arg.coef = flipdims(arg.coef); % trick
x = Cdiff1_ml_filt_forw_Fatrix(arg, y);


% Cdiff1_ml_sp_setup()
% setup for sparse version
%
function C = Cdiff1_ml_sp_setup(arg)
...
% both [*N (L)] or [(N) (L)]
%
function y = Cdiff1_ml_sp_forw(arg, x)
...
% both [*N (L)] or [(N) (L)]
%
function x = Cdiff1_ml_sp_back(arg, y)
